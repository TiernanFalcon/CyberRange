<#
.SYNOPSIS
    This PowerShell script configures the Remote Desktop Session Host to require
    secure RPC communications by setting the fEncryptRPCTraffic registry value.
...
.USAGE
    ...
    then enable secure RPC for Remote Desktop Services per STIG requirement
    WN11-CC-000285. Requiring secure RPC communications enhances security for
    Remote Desktop connections by only allowing authenticated and encrypted requests.
...
try {
    ...
    # Output result
    Write-Host "Successfully configured Remote Desktop secure RPC setting:" -ForegroundColor Green
